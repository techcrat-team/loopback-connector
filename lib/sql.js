...
SQLConnector.prototype.fromRow = SQLConnector.prototype.fromDatabase =
  function (model, modelProperties, rowData) {
    if (rowData == null) {
      return rowData;
    }

    var data = {};
    for (var p in modelProperties) {
      var columnName = modelProperties[p].__columnName;
      // Load properties from the row
      var columnValue = this.fromColumnValue(modelProperties[p], rowData[columnName]);
...
SQLConnector.prototype.all = function find(model, filter, options, cb) {
  ...
  this.execute(stmt.sql, stmt.params, options, function (err, data) {
    ...
    var modelProperties = self.getModelDefinition(model).properties;
    for (var property in modelProperties) {
      modelProperties[property].__columnName = self.column(model, property);
    }
    var objs = data.map(function (obj) {
      return self.fromRow(model, modelProperties, obj);
    });
